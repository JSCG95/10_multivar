...
weather %>%
  ...
  geom_point() +
  geom_smooth(method="lm") +
  stat_correlation() # plots R value
```
...
- Algorithm that handles multicollinearity  
    - Variable selection (internal of a model - LASSO).     
...
Principal components = Linear features. PC same number as original features. 
...
- PC are not multicollinear, orthogonal, independent. 

- The larger the eigenvector
- PC1 always the most variability. 
```{r pca in action}
...
scale. = T --> Scale variables within the function. Critical step if not scaling before. 
...
How many PCs we are going to use. 
...
- Useful to reduce the number of arrows to only the variables that are contributing the most--> select.var = list(contrib = 5)
...
k-means does not normalize our data for us like PCA did, so we will need to do that before running the model. Variables magnitude matter (larger variables, higher impact). 

across(columns where to apply, function to apply)
...
Any thoughts?  
Let's try 2.  k (pull 6 random observations and set as centers)
nstart? Randomness,nstart run k means 10 times to avoid impact of choosing the same values randomly. 
```{r kmeans model }
...
mod_km
```
(between_SS / total_SS =  58.5 %) --> How much variability we are explaining with those clusters. 
...
The elbow method --> Choose number of K where you see an elbow. In this case, between 3 - 5. Minimum number of clusters that explain most of the information. 
...
fviz_nbclust(weather_norm,
             ...
             FUNcluster = kmeans)
```
Silhoutte widht --> Average width heights as possible. Observations in that cluster have a larger level of similarity within the cluster.
...
```
Based on both cluster methods 4 is one of the recommendations. 
...
Tally() shortcut for sample size/observations count
```{r}
weather %>%
...
Which variables can differentiate clusters. 
